...
def sum_of_kinch(source: list, result_json: dict) -> float:
    total_sum = 0
    for item in source:
        if item in result_json and "3bld" in result_json[item]:
            total_sum += 1 / result_json[item]["3bld"]
    return total_sum
...
def main():
    def crawl_spreadsheet(spreadsheet: gs.Spreadsheet, name: str, isInverse: bool) -> None:
        ...
        while True:
            try:
                ...
                logger.warning(e + " when getting the worksheets.")
                time.sleep(10)
        ...
        for worksheet in worksheets:
            ...
            while True:
                try:
                    ...
                    logger.warning(e + " when trying to get values from the spreadsheet.")
                    time.sleep(10)
            isbigbld = False
            for rows in values:
                for cell in rows:
        ...
        for key in url_json[name]:
            ...
            while True:
                try:
                    ...
                    logger.warning(e + " when opening the spreadsheet.")
                    time.sleep(10)
    ...
    for output_type in output_types:
        output_file = "public/data/" + output_type + "Manmade.json"
