...
function setSelect(alg) {
    ...
    if (cornerstylecookie === "manmade") {
        const singleList = [];
        for (const algs of cornerAlgToInfoStyle[standardAlg]) {
    ...
    $(`#select-algorithm-${standardAlg}`).selectize( {
        ...
        "onChange" (algorithm) {
            ...
            localStorage.setItem(`corner-${standardAlg}`, simplifyValue);
...
function setLocal() {
    for (const alg of algList) {
        const standardAlg = cornerAlgToStandard[alg];
        const selectize = $(`#select-algorithm-${standardAlg}`).selectize()[0].selectize;
        const localValue = localStorage.getItem(`corner-${standardAlg}`);
...
function clearSheet() {
    for (const alg of algList) {
        const standardAlg = cornerAlgToStandard[alg];
        localStorage.removeItem(`corner-${standardAlg}`);
    }
    algSearch();
}
